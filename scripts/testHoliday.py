...
#    testHoliday.py
# Usage:
#    python testHoliday.py <input_file> <output_file>
# Description:
...
def writeResult(output,rawData,calcData,p5):
    ...
    result = np.array(np.empty(rawData.shape[0]), dtype=[("timeStamp","|S19"),("dateFrac",float),("isHoliday",int),("isSunday",int),("cnt",int),("predSimple",int),("predTrig",int),("predHourDay",int),("predHourWeek",int),("predHoliday",int)])
...
PH = getPredictions(trainData,hourlyData,"modelHoliday")
writeResult(outputFileName,trainData,hourlyData,PH)
